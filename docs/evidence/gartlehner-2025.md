> **THIS FILE IS GENERATED. DO NOT EDIT DIRECTLY.**

# Artificial Intelligence-Assisted Data Extraction With a Large Language Model: A Study Within Reviews

## Bibliographic information

- **Authors:** Gerald Gartlehner, Shannon Kugley, Karen Crotty, Meera Viswanathan, Andreea Dobrescu, Barbara Nussbaumer-Streit, Graham Booth, Jonathan R. Treadwell, Jung Min Han, Jesse Wagner, Eric A. Apaydin, Erin L. Coppola, Margaret Maglione, Rainer Hilscher, Robert Chew, Meagan Pilar, Bryan Swanton, Leila C. Kahwati
- **Year:** 2025
- **DOI:** 10.7326/ANNALS-25-00739
- **URL:** https://doi.org/10.7326/ANNALS-25-00739
- **Publication status:** Published in Annals of Internal Medicine 178(12), 1763-1771
- **Source category:** peer-reviewed-study
- **Record type:** journal-article
- **Notes:** Prospective study within six ongoing reviews comparing Claude-first extraction checked by a human with conventional human-first extraction checked by a second human. Across 9,341 data items from 63 studies, the AI-assisted workflow had 91.0% accuracy versus 89.0% for human-only extraction and saved a median 41 minutes per study. Both arms retained errors, and only discordant items were adjudicated, so shared errors could have been missed.

## Linked claims

- [In one prospective study within reviews, human-verified LLM extraction was slightly more accurate and faster than a conventional human-only workflow, while both approaches retained consequential errors.](../claims/extraction-001.md) — **supports**
- [Current LLM data extraction can be precise yet incomplete, with omissions as the dominant error; task-specific prompts improve recall but do not remove the need for human verification of meta-analytic data.](../claims/extraction-002.md) — **contextual**
