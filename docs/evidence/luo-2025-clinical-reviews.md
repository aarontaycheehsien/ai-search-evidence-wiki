> **THIS FILE IS GENERATED. DO NOT EDIT DIRECTLY.**

# Cross sectional pilot study on clinical review generation using large language models

## Bibliographic information

- **Authors:** Zining Luo, Yang Qiao, Xinyu Xu, Xiangyu Li, Mengyan Xiao, Aijia Kang, Dunrui Wang, Yueshan Pang, Xing Xie, Sijun Xie, Dachen Luo, Xuefeng Ding, Zhenglong Liu, Ying Liu, Aimin Hu, Yixing Ren, Jiebin Xie
- **Year:** 2025
- **DOI:** 10.1038/s41746-025-01535-z
- **URL:** https://doi.org/10.1038/s41746-025-01535-z
- **Publication status:** Published in npj Digital Medicine 8:170
- **Source category:** peer-reviewed-study
- **Record type:** journal-article
- **Notes:** Compared 2,169 AI-generated clinical reviews with 62 reviews from four leading medical journals. Reference accuracy was the percentage of cited references whose explanation in a review was manually judged consistent with the original article. The pooled AI median was 100% (IQR 73.55-100%) versus 100% (IQR 100-100%) for human reviews; standalone LLMs had a median of 95% (IQR 51.22-100%). Results varied substantially across systems and generation workflows, and the study did not cover all clinical fields.

## Linked claims

- [Verifying that an LLM cites real papers is insufficient to establish evidence fidelity because the generated claims may still misstate, overstate, or contradict those papers.](../claims/citation-001.md) — **supports**
- [Reported citation-fidelity rates for LLM-generated scientific syntheses are sensitive to the evaluation protocol, so comparisons require the same unit of analysis, support rubric, verifier, and source-access procedure.](../claims/citation-002.md) — **contextual**
