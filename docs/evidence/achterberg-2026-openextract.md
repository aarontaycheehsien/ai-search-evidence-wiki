> **THIS FILE IS GENERATED. DO NOT EDIT DIRECTLY.**

# OpenExtract: Automated Data Extraction for Systematic Reviews in Health

## Bibliographic information

- **Authors:** Jim Achterberg, Bram van Dijk, Jing Meng, Saif Ul Islam, Gregory Epiphaniou, Carsten Maple, Xuefei Ding, Theodoros N. Arvanitis, Simon Brouwer, Marcel Haas, Marco Spruit
- **Year:** 2026
- **DOI:** 10.3233/SHTI260316
- **URL:** https://doi.org/10.3233/SHTI260316
- **Publication status:** Published in Studies in Health Technology and Informatics, pp. 934-938
- **Source category:** peer-reviewed-study
- **Record type:** conference-paper
- **Notes:** Evaluated a retrieval-augmented extraction pipeline on 10 papers and 150 categorical data points from a digital-health systematic review. Against a human-consensus comparison, aggregate precision/recall were 0.820/0.820 for DeepSeek V3.1, 0.846/0.813 for Qwen2.5 72B, and 0.624/0.564 for Qwen2.5 7B. The evaluation subset was small and text-only parsing missed table/figure content; the paper reports no extraction-time benchmark.

## Linked claims

- [LLM extraction performance varies by field and structural complexity: explicit study descriptors are generally easier than numerical outcomes, relational bindings, and complete synthesis-ready records.](../claims/extraction-003.md) — **supports**
